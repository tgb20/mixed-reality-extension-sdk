...
export class WebHost {
	...
	public constructor(
		...
		const serverP = this.validateManifest().then(() =>
		...
		serverP
			.then((server) => {
				log.info(
					"app",
					`${server.name} listening on ${JSON.stringify(
						server.address()
					)}`
				);
				...
				if (options.endpoints) {
					options.endpoints.forEach((endpoint) => {
						if (endpoint.type === RequestType.GET) {
							server.get(endpoint.url, endpoint.handler);
						} else {
							server.post(endpoint.url, endpoint.handler);
						}
					});
				}
				...
				if (this.baseDir) {
					server.get(
						"/*",
						Restify.plugins.serveStaticFiles(this._baseDir)
					);
				}
			})
			.catch((reason) =>
				log.error("app", `Failed to start HTTP server: ${reason}`)
			);
	}
	...
	private checkStaticBuffers = (
		...
		if (info) {
			...
			next(
				new NotFoundError(
					`No buffer registered under name ${req.params.name}`
				)
			);
		}
	};
	...
	private async validateManifest() {
		...
		if (!result.valid) {
			log.error(
				"app",
				`App manifest "${manifestPath}" is not valid:\n${result.errors.join(
					"\n"
				)}`
			);
			this.manifest = null;
		...
		if (!this.manifest && !permissions && !optionalPermissions) {
			log.warning(
				...
				"No MRE manifest provided, and no permissions requested! For this MRE to use protected features, " +
					`provide an MRE manifest at "${resolve(
						this.baseDir,
						"./manifest.json"
					)}", or pass a permissions ` +
